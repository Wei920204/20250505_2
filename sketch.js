...
function preload() {
  earImg = loadImage('ear.png'); // 預先載入耳朵圖片
}
...
function draw() {
    ...
    image(video, 0, 0);  //顯示你的畫面在螢幕上
      ...
      // 顯示左耳圖片
      let leftEar = pose.keypoints[3]; // 左耳的 keypoint
      if (leftEar && leftEar.score > 0.2) { // 確保偵測到左耳且可信度高
        image(earImg, leftEar.position.x - 16, leftEar.position.y - 16, 32, 32); // 顯示耳朵圖片
      }
